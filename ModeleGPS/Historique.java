...
public class Historique {
	...
	// Methode qui permet de supprimer des parametres de l'historique
		public void supprimerParametres(Parametres parametres) {
			this.listeDeParametres.remove(parametres);
		}
	...
	public Boolean contientVehicule(Vehicule v) {
		for(int i = 0; i<this.listeDesVehicules.size(); i++) {
			if(v.getNom().equals(this.listeDesVehicules.get(i).getNom()) && v.getCarburant() == this.listeDesVehicules.get(i).getCarburant() && v.getGabarit() == this.listeDesVehicules.get(i).getGabarit()) {
				return true;
			}
		}
		return false;
